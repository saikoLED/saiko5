...
{
    size_t result = lo_validate_string(data, size);
    return (result >= 4) ? (char *)data : NULL;
}

size_t lo_validate_string(void *data, size_t size)
{
    size_t i = 0, len = 0;
    char *pos = data;
...
size_t lo_validate_blob(void *data, size_t size)
{
    size_t i, end, len;
    uint32_t dsize;
    char *pos = (char *)data;
...
size_t lo_validate_bundle(void *data, size_t size)
{
    size_t len = 0, remain = size;
    char *pos = data;
    uint32_t elem_len;
...
size_t lo_validate_arg(lo_type type, void *data, size_t size)
{
    if (size < 0) {
        return -1;
    }
